...
public class JobInfoController {
    ...
    @GetMapping()
    public ReturnT<Map<String, Object>> index(HttpServletRequest request, @RequestParam(required = false, defaultValue = "-1") int jobGroup) {
        Map<String, Object> model = new HashMap<>(8);
        // 枚举-字典
        model.put("ExecutorRouteStrategyEnum", ExecutorRouteStrategyEnum.values());        // 路由策略-列表
        model.put("GlueTypeEnum", GlueTypeEnum.values());                                // Glue类型-字典
        model.put("ExecutorBlockStrategyEnum", ExecutorBlockStrategyEnum.values());        // 阻塞处理策略-字典
        model.put("ScheduleTypeEnum", ScheduleTypeEnum.values());                        // 调度类型
        model.put("MisfireStrategyEnum", MisfireStrategyEnum.values());                    // 调度过期策略

        // 执行器列表
        List<XxlJobGroup> jobGroupDaoAll = xxlJobGroupDao.findAll();

        // filter group
        List<XxlJobGroup> jobGroupList = filterJobGroupByRole(request, jobGroupDaoAll);
        if (jobGroupList == null || jobGroupList.size() == 0) {
            throw new XxlJobException(I18nUtil.getString("jobgroup_empty"));
        }

        model.put("JobGroupList", jobGroupList);
        model.put("jobGroup", jobGroup);

        return new ReturnT<>(model);
    }
    ...
    public static List<XxlJobGroup> filterJobGroupByRole(HttpServletRequest request, List<XxlJobGroup> xxlJobGroups) {
        ...
        if (xxlJobGroups != null && xxlJobGroups.size() > 0) {
            ...
            if (loginUser.getRole() == 1) {
                ...
                for (XxlJobGroup groupItem : xxlJobGroups) {
